...
jboss.directive('selectAddress', function($http, $q, $compile,BaseService) {
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            return delay.promise.then(function(data) {
                ...
                scope.$watch('p', function(newV) {
                    ...
                    if (newV) {
                        ...
                        for (_i = 0, _len = data.length; _i < _len; _i++) {
                            v = data[_i];
                            if (v.proviceName === newV) {
                                _results.push(scope.cities = v.cityBaseInfos);
                            }
                        }
                        return _results;
                    } else {
                ...
                scope.$watch('c', function(newV) {
                    ...
                    if (newV) {
                        ...
                        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                            v = _ref[_i];
                            if (v.cityName === newV) {
                                _results.push(scope.dists = v.districtBaseInfos);
                            }
                        }
                ...
                scope.$watch('a', function(newV) {
                   if(newV){

                       scope.$parent.fm.areaId = newV;
                       scope.isAuth = false;
